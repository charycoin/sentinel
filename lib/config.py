...
sentinel_cfg = CharyCoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_charycoind_proto_version_with_sentinel_ping = 70207


def get_charycoin_conf():
    home = os.environ.get('HOME')

    charycoin_conf = os.path.join(home, ".charycoincore/charycoin.conf")
    if sys.platform == 'darwin':
        charycoin_conf = os.path.join(home, "Library/Application Support/CharyCoinCore/charycoin.conf")

    charycoin_conf = sentinel_cfg.get('charycoin_conf', charycoin_conf)

    return charycoin_conf
...
charycoin_conf = get_charycoin_conf()
network = get_network()
db = get_db_conn()
