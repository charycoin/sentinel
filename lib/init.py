...
def has_charycoin_conf():
    import config
    import io

    valid_charycoin_conf = False

    # ensure charycoin_conf exists & readable
    #
    # if not, print a message stating that CharyCoin Core must be installed and
    # configured, including JSONRPC access in charycoin.conf
    try:
        f = io.open(config.charycoin_conf)
        valid_charycoin_conf = True
    except IOError as e:
        print(e)

    return valid_charycoin_conf
...
def main():
    ...
    if not has_charycoin_conf():
        print("CharyCoinCore must be installed and configured, including JSONRPC access in charycoin.conf")
        sys.exit(1)
