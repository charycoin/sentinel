...
def is_valid_charycoin_address(address, network='mainnet'):
    ...
    charycoin_version = 140 if network == 'testnet' else 28
    ...
    try:
        ...
        # rescue from exception, not a valid CharyCoin address
        return False

    if (address_version != charycoin_version):
        return False
...
# shims 'til we can fix the charycoind side
def SHIM_serialise_for_charycoind(sentinel_hex):
    ...
    # superblock => "trigger" in charycoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # charycoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    charycoind_hex = serialise(obj)
    return charycoind_hex


# shims 'til we can fix the charycoind side
def SHIM_deserialise_from_charycoind(charycoind_hex):
    from models import TALCASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(charycoind_hex)

    # shim from charycoind
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('charycoin.conf')
