...
class CharyCoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some charycoind methods
    ...
    @classmethod
    def from_charycoin_conf(self, charycoin_dot_conf):
        from charycoin_config import CharyCoinConfig
        config_text = CharyCoinConfig.slurp_config_file(charycoin_dot_conf)
        creds = CharyCoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = charycoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = charycoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = charycoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_charycoind_proto_version_with_sentinel_ping)
