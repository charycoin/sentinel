...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, charycoind):
        golist = charycoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_charycoind(charycoind, item)
    ...
    @classmethod
    def import_gobject_from_charycoind(self, charycoind, rec):
        import decimal
        import charycoinlib
        import inflection
        ...
        # shim/charycoind conversion
        object_hex = charycoinlib.SHIM_deserialise_from_charycoind(object_hex)
        objects = charycoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from charycoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from charycoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from charycoind - CharyCoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(charycoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from charycoind! %s" % e)
            govobj.vote_delete(charycoind)
            return (govobj, None)
    ...
    def vote_delete(self, charycoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(charycoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, charycoind, signal, outcome):
        import charycoinlib
        ...
        output = charycoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = charycoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(charycoind, signal)

    def sync_network_vote(self, charycoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = charycoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 charycoin addr, non-multisig
            if not charycoinlib.is_valid_charycoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid CharyCoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (charycoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CharyCoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import charycoinlib
        obj_data = charycoinlib.SHIM_serialise_for_charycoind(self.serialise())
    ...
    def prepare(self, charycoind):
        try:
            object_hash = charycoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not charycoinlib.is_valid_charycoin_address(addr, config.network):
    ...
    def hash(self):
        import charycoinlib
        return charycoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, charycoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - charycoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, charycoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - charycoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, charycoind):
        now = int(time.time())
        return (self.created_at < (now - charycoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, charycoind):
        if self.is_expired(charycoind):
            return False

        return True

    def is_deletable(self, charycoind):
        if self.is_expired(charycoind):
            return True
