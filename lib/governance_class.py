...
class GovernanceClass(object):
    ...
    def vote(self, charycoind, signal, outcome):
        return self.go.vote(charycoind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, charycoind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(charycoind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(charycoind, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to charycoind "gobject list" output
    ...
    def charycoind_serialise(self):
        import charycoinlib
        return charycoinlib.SHIM_serialise_for_charycoind(self.serialise())
