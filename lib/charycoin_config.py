...
class CharyCoinConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read charycoin.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is charycoin.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from charycoin.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard CharyCoin defaults...
        default_port = 11050 if (network == 'mainnet') else 21050

        # use default port for network if not specified in charycoin.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from charycoin.conf
