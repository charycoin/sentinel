...
def test_valid_charycoin_address():
    from charycoinlib import is_valid_charycoin_address

    main = valid_charycoin_address()
    test = valid_charycoin_address('testnet')

    assert is_valid_charycoin_address(main) is True
    assert is_valid_charycoin_address(main, 'mainnet') is True
    assert is_valid_charycoin_address(main, 'testnet') is False

    assert is_valid_charycoin_address(test) is False
    assert is_valid_charycoin_address(test, 'mainnet') is False
    assert is_valid_charycoin_address(test, 'testnet') is True


def test_invalid_charycoin_address():
    from charycoinlib import is_valid_charycoin_address

    main = invalid_charycoin_address()
    test = invalid_charycoin_address('testnet')

    assert is_valid_charycoin_address(main) is False
    assert is_valid_charycoin_address(main, 'mainnet') is False
    assert is_valid_charycoin_address(main, 'testnet') is False

    assert is_valid_charycoin_address(test) is False
    assert is_valid_charycoin_address(test, 'mainnet') is False
    assert is_valid_charycoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from charycoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from charycoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % charycoinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(charycoinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(charycoinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(charycoinlib.blocks_to_seconds(16616)) == 2612035
